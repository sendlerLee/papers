...
def main(unused_argv):
  ...
  # Set up training hook that logs the training accuracy every 100 steps.
    #tensors_to_log = {'train_auc': 'train_auc','cross_entropy': 'cross_entropy'}
    #logging_hook = tf.train.LoggingTensorHook(
    #  tensors=tensors_to_log, every_n_iter=100)

  # Train the model
    for n in range(FLAGS.train_epochs // FLAGS.epochs_per_eval):
        _classifier.train(input_fn=lambda :input_fn(model_params.data_dir + "/train", model_params.batch_size))#, hooks=[logging_hook])

        eval_results = _classifier.evaluate(input_fn=lambda :input_fn(model_params.data_dir + "/test",model_params.batch_size))
        print()
